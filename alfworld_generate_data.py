...
ITER = 20 # Sampling Iterations (whether it's appropriate?)
...
def parse_opt():
    ...
    parser.add_argument("--disable_closeloop", action="store_true", default=False, help="Whether disable closed-loop refinement in data generation")
...
save_to_folder = './data/alfworld/gpt3.5'
# save_to_folder = f'{save_folder}/r_white{whitebox}_black{GPT_MODEL}_usegpt{args.use_gpt}_{datetime}/'
...
DPO_PAIR = [] # a list of {'prompt': prompt, 'task': task, 'receptacle': receptacle_list, 'positive': [positive pairs], 'negative': [negative pairs]} used for dpo training
DPO_FILE = 'train.json'
for iteration in range(ITER):
    ...
    for _ in range(game_num):
        ...
        game_dict = {}
        game_dict['prompt'] = dpo_prompt
        game_dict['task'] = task
        game_dict['receptacle'] = receptacle_list
        game_dict['positive'] = []
        game_dict['negative'] = []
        ...
        for num_try in range(NUM_TRY):
            ...
            if agent.is_success:
                game_dict['positive'].append({'llama_decompose': decomposition, 'gpt_solution': solution_func, 'response': parse_solution_to_decompose(solution_func)})
                # SFT_PAIR.append({'prompt': dpo_prompt, 'general_response': parse_solution_to_decompose(solution_func), 'response': parse_solution_to_decompose(solution_func, add_general=False)})
                # SFT_BACKUP_PAIR.append({'prompt': dpo_prompt, 'general_response': parse_solution_to_decompose(solution_func), 'response': parse_solution_to_decompose(solution_func, add_general=False), 'solution': solution_func, 'task': task})
                break
            ...
            save_to_file(save_to_folder, EXEC_FILE, f'Task {_+1} Try{num_try+1}:\n' + f'Executing solution function with start_from={step}:\n' + solution_func + '\n' + 'Error Message:\n' + error_msg)
            if num_try == 0:
                game_dict['negative'].append({'llama_decompose': decomposition, 'gpt_solution': solution_func, 'error_msg': error_msg, 'response': decomposition})
            error_step = get_error_step(error_string)

            # if disabled close-loop, we only iterate for one iteration
            if args.disable_closeloop:
                break
        ...
        # save result
        DPO_PAIR.append(game_dict)
        backup_folder = os.path.join(save_to_folder, 'collect_backup')
        os.makedirs(backup_folder, exist_ok=True)
        with open(os.path.join(backup_folder, DPO_FILE), 'w') as f:
...
def wrap_dataset(prompt: str, response: str) -> dict:
    chats = []
...
backup_folder = os.path.join(save_to_folder, 'collect_backup')
json_file = os.path.join(backup_folder, DPO_FILE)
with open(json_file, "r") as f:
    dpo_pairs = json.load(f)

jsons_file = os.path.join(save_to_folder, 'collect', 'train.jsonl')
os.makedirs(os.path.join(save_to_folder, 'collect'), exist_ok=True)
with open(jsons_file, "w") as f:
    for data in dpo_pairs:
        ...
        positive_decompose = [wrap_dataset(data["prompt"], d["response"]) for d in positive_decompose]
        negative_decompose = random.sample(data["negative"], cnt)
        negative_decompose = [wrap_dataset(data["prompt"], d["response"]) for d in negative_decompose]
