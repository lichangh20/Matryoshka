...
from .backend import call_gpt, call_chat_gpt, GPT_MODEL, call_whitebox
...
class ProgramChatInterface(ProgramInterface):
    ...
    def generate(self, prompt: str, temperature: float = 0, top_p: float = 1, max_tokens: int = 512):
        ...
        gen = self.gpt.call_chat_gpt(messages, stop=self.stop, temperature=temperature, top_p=top_p, max_tokens=max_tokens)
        if self.verbose:
            print(gen)
    ...
    def decompose(self, decompose_message, client, model, prompt: str, temperature: float = 0, top_p: float = 1, max_tokens: int = 512, num_returns: int = 1):
        messages = [{'role': 'system', 'content': decompose_message}, {'role': 'user', 'content': prompt}]
        return call_whitebox(messages, client, model, temperature=temperature, top_p=top_p, max_tokens=max_tokens, num_returns = num_returns)